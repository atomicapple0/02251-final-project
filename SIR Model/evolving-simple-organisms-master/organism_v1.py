...
settings['gens'] = 1           # number of generations
settings['elitism'] = 0.20      # elitism (selection bias)
settings['mutate'] = 0.10       # mutation rate

# SIMULATION SETTINGS
settings['gen_time'] = 4      # generation length         (seconds)
...
def plot_frame(settings, organisms, foods, gen, time):
    ...
    multiplier = 0.1
    plt.xlim([settings['x_min'] + settings['x_min'] * multiplier, settings['x_max'] + settings['x_max'] * multiplier])
    plt.ylim([settings['y_min'] + settings['y_min'] * multiplier, settings['y_max'] + settings['y_max'] * multiplier])
